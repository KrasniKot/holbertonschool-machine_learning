...
class BidirectionalCell:
    ...
    def backward(self, h_next, x_t):
        """ calculates the hidden state in the backward direction
            ...
            - x_t: ndarray, shape(m, i) data input
        """
        hnextxt = np.concatenate((h_next, x_t), axis=1)

        h_back = np.tanh(hnextxt @ self.Whb + self.bhb)

        return h_back

    def output(self, H):
        """ calculates all outputs for the RNN
            - H: ndarray, shape(t, m, 2*h), concatenated hidden states
                 from both directions excluding initialized states
        """
        # Dimension extracction
        t, m, _ = H.shape
        ...
        for step in range(t):
            ...
            Y[step] = self._softmax(H[step] @ self.Wy + self.by)
