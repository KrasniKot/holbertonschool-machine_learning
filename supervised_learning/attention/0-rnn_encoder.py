...
class RNNEncoder(layers.Layer):
    ...
    def __init__(self, vocab, embedding, units, batch):
        ...
        self.batch     = batch
        self.units     = units
        self.embedding = layers.Embedding(input_dim=vocab, output_dim=embedding)
        self.gru       = layers.GRU(units=self.units, return_sequences=True, return_state=True, recurrent_initializer='glorot_uniform')
    ...
    def call(self, x, initial):
        """ Passes the parameters to the gru function
            - x ........ tensor of shape (batch, input_seq_len) containing the input to the encoder
                         layer as word indices within the vocabulary
            - initial .. tensor of shape (batch, units), contains the initial hidden state
        """
