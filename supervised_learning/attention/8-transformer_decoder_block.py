...
class DecoderBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, dm, h, hidden, drop_rate=0.1):
        ...
        # Instance attributes
        self.mha1 = MultiHeadAttention(dm, h)  # Autoregressive attention
        self.mha2 = MultiHeadAttention(dm, h)  # Encoder's output attention
    ...
    def call(self, x, encoder_output, training, look_ahead_mask, padding_mask):
        ...
        # apply a skip connection to preserve information from the original
        xmmattentions = self.layernorm1(x + mmattentions)

        # Apply the second attention mechanism to enable the decoder to
        # integrate information from the encoder's output while generating
        encattentions, _ = self.mha2(xmmattentions, encoder_output,
