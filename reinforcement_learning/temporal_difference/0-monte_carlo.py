...
def monte_carlo(env, V, policy, episodes=5000, max_steps=100, alpha=0.1,
    ...
    for episode in range(episodes):  # Loop over the episodes
        ...
        for _ in range(max_steps):
            ns, r, term, trunc, _ = env.step(policy(state))  # 2. Perform an
        ...
        for state, reward in edata[::-1]:
            ...
            if state not in edata[:episode, 0]:  # Check state is not already
                V[state] = V[state] + alpha * (G - V[state])  # Update value
        ########
