...
def hierarchy(df1, df2):
    """ Rearranges the MultiIndex of concatenated DataFrames and filters rows based on a timestamp range """  # noqa
    # Filter both DataFrames for the specified timestamp range
    df1 = df1.loc[(df1['Timestamp'] >= 1417411980) & (df1['Timestamp'] <= 1417417980)]  # noqa
    df2 = df2.loc[(df2['Timestamp'] >= 1417411980) & (df2['Timestamp'] <= 1417417980)]  # noqa
    ...
    concatenated = pd.concat([dff2, dff1], keys=['bitstamp', 'coinbase'])  # noqa
